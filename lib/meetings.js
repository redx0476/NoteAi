...
// Every column serialize() reads — kept explicit so queries never drag along
// columns the response doesn't use.
const MEETING_COLS =
  'id, title, platform, meeting_url, status, summary, objectives, action_items, chapters, keywords, participants, has_audio, audio_object_key, started_at, ended_at';
...
function serialize(m) {
  return {
    ...
    status: m.status,
    summary: m.summary,
    objectives: asArray(m.objectives),
    ...
    audioObjectKey: m.audio_object_key || null,
...
async function getMeeting(id, userId) {
  const [meeting, seg, hl] = await Promise.all([
    pool.query(`SELECT ${MEETING_COLS} FROM meetings WHERE id = $1 AND user_id = $2`, [id, userId]),
    pool.query(
      'SELECT speaker, text, t_offset AS "tOffset", created_at FROM segments WHERE meeting_id = $1 ORDER BY id',
      [id]
    ),
    pool.query(
      'SELECT id, speaker, text, t_offset AS "tOffset", created_at FROM highlights WHERE meeting_id = $1 ORDER BY t_offset, id',
      [id]
    ),
  ]);
...
module.exports = { serialize, getMeeting, ownsMeeting, fmtTs, MEETING_COLS };
