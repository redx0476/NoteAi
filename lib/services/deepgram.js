...
function openDeepgram({ onTranscript, onError, onClose, onOpen }) {
  ...
  const params = new URLSearchParams({
    ...
    encoding: 'linear16',
    sample_rate: '16000',
    // Two interleaved channels: ch0 = the local mic ("You"), ch1 = the meeting
    // tab. multichannel keeps them transcribed separately; diarize still splits
    // multiple remote speakers within the meeting channel.
    channels: '2',
    multichannel: 'true',
    smart_format: 'true',
    punctuate: 'true',
