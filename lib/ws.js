...
const { openRecording } = require('./audio');
...
async function authFromUrl(reqUrl) {
  ...
  return { meetingId, userId: payload.uid, path: url.pathname };
}
...
function handleLive(ws, { meetingId }) {
  ...
  ws.on('close', () => subscribers(meetingId).delete(ws));
  ws.on('error', () => subscribers(meetingId).delete(ws));
}

// ── Audio ingest (streaming recorder) ────────────────────────────────────────
function handleIngest(ws, { meetingId }) {
  subscribers(meetingId).add(ws);
  ...
  const recording = openRecording(meetingId);
  ...
  const writeAudio = (data) => {
    ...
    if (!audioMarked) {
      audioMarked = true;
      pool.query('UPDATE meetings SET has_audio = true WHERE id = $1', [meetingId]).catch(() => {});
    }
  };
  ...
  if (!deepgramEnabled()) {
    ...
    try {
      dg = openDeepgram({
        ...
        onTranscript: ({ text, isFinal, speaker, start }) => {
          const payload = {
  ...
  const cleanup = () => {
    clearTimeout(readyTimer);
    subscribers(meetingId).delete(ws);
    try {
      recording.end();
    } catch {
      /* ignore */
